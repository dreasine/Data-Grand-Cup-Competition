...
import pandas as pd
import numpy as np
from data_helper import preprocess,preprocess_char
...
x_train, y_train, vocab_processor, x_val, y_val,x_test = preprocess_char()
print(x_train.shape)
...
embed = Embedding(config.vocab_size + 1, 256, input_length=config.seq_length)(main_input)
# 词窗大小分别为3,4,5
cnn1 = Convolution1D(128, 1, padding='same', strides = 1, activation='relu')(embed)
cnn1 = MaxPool1D(pool_size=4)(cnn1)


cnn2 = Convolution1D(128, 2, padding='same', strides = 1, activation='relu')(embed)
cnn2 = MaxPool1D(pool_size=4)(cnn2)


cnn3 = Convolution1D(128, 3, padding='same', strides = 1, activation='relu')(embed)
cnn3 = MaxPool1D(pool_size=4)(cnn3)


cnn4 = Convolution1D(128, 4, padding='same', strides = 1, activation='relu')(embed)
cnn4 = MaxPool1D(pool_size=4)(cnn4)

cnn5 = Convolution1D(128, 7, padding='same', strides = 1, activation='relu')(embed)
cnn5 = MaxPool1D(pool_size=4)(cnn5)
# 合并三个模型的输出向量
'''
...
test_prob.to_csv('result/prob/prob_textcnn_char.csv',index=None)
...
test_pred[["id","class"]].to_csv('result/textcnn_char.csv',index=None)