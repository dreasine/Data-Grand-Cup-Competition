...
def preprocess_char():
    ...
    # Build vocabulary
    #max_document_length = max([len(x.split(" ")) for x in x_text])
    vocab_processor = learn.preprocessing.VocabularyProcessor(1500,100)
    #x = np.array(list(vocab_processor.fit_transform(x_text)))
    ...
    vocab_processor.save(os.path.join(vocab_dir, "vocab_char"))
...
def preprocess2():
    ...
    # Build vocabulary
    #max_document_length = max([len(x.split(" ")) for x in x_text])
    vocab_processor = learn.preprocessing.VocabularyProcessor(2000,100)
    #x = np.array(list(vocab_processor.fit_transform(x_text)))
