...
conv = Conv1D(64, kernel_size = 2, padding = "valid", kernel_initializer = "he_uniform")(lstm)
avg_pool = GlobalAveragePooling1D()(conv)
max_pool = GlobalMaxPooling1D()(conv)
x = concatenate([avg_pool, max_pool])
main_output = Dense(config.num_classes, activation='softmax')(x)


'''
# 词窗大小分别为3,4,5
cnn1 = Convolution1D(256, 3, padding='same', strides = 1, activation='relu')(lstm)
cnn1 = MaxPool1D(pool_size=4)(cnn1)
cnn2 = Convolution1D(256, 4, padding='same', strides = 1, activation='relu')(lstm)
cnn2 = MaxPool1D(pool_size=4)(cnn2)
cnn3 = Convolution1D(256, 5, padding='same', strides = 1, activation='relu')(lstm)
cnn3 = MaxPool1D(pool_size=4)(cnn3)
cnn4 = Convolution1D(256, 8, padding='same', strides = 1, activation='relu')(lstm)
...
test_prob.to_csv('result/prob/prob_long_textcnn_test.csv',index=None)
